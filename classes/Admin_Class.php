...
class Admin_Class
{
	...
	{ 
        //$host_name='localhost';
		//$db_name='u754249269_etms_db';
		//$user_name='u754249269_eprecision';
		//$password='Server_101';
...
/*-----------------beginTransaction()-----------------*/
	...
	public function getBreakEndTime($user_id) {
		try {
			$stmt = $this->db->prepare("SELECT break_end_time FROM break_info WHERE br_user_id = :user_id ORDER BY break_end_time DESC LIMIT 1");
			$stmt->bindParam(':user_id', $user_id);
			$stmt->execute();
			$row = $stmt->fetch(PDO::FETCH_ASSOC);
			return $row['break_end_time'];
...
/* ---------------------- test_form_input_data ----------------------------------- */
	...
	/*---------------isUserClockedIn()----------------*/
	public function isUserClockedIn($user_id) {
		// Implement your logic to check if the user is clocked in here
		// For example, you might query the database to check if there's an active attendance record for the user
	
		// Assuming you have a method in Admin_Class to check if the user is clocked in, you would call it like this:
		// Modify this code according to your database schema and logic
...
/*----------- add_new_user--------------*/
	...
	public function add_new_admin($data){
		...
		$user_email = $this->test_form_input_data($data['ad_email']);
		$temp_password = rand(000000001,10000000);
		$user_password = $this->test_form_input_data(md5($temp_password));
		$user_role = 1;
		try{
